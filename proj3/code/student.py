...
def get_tiny_images(image_paths):
    ...
    dim = 8
    # 设定dim为8表示将读取的图像resize成8*8的小图像
    tiny_images = np.empty((len(image_paths), dim * dim), dtype=float)
    for i in range(0, len(image_paths)):
        ...
        im = rgb2grey(im)
        # 缩小图像
        im = resize(im, (dim, dim), anti_aliasing=True)
        # 将图像像素归一化
        im = im / np.sum(im)
        tiny_images[i, :] = np.reshape(im, (dim * dim))
...
def get_hogs(image, cells_per_block, pixels_per_cell, orients):
    # 缩小图像以进行调试
    scale = 0.4
    image = resize(image, (int(image.shape[0] * scale), int(image.shape[1] * scale)), anti_aliasing=True)

    out = hog(image, orients, cells_per_block=cells_per_block, pixels_per_cell=pixels_per_cell, feature_vector=True,
              block_norm='L1')

    return out.reshape(-1, cells_per_block[0] * cells_per_block[1] * orients)
...
def build_vocabulary(image_paths, vocab_size):
    ...
    out = np.empty((0, cells_per_block[0] * cells_per_block[1] * orients))
    # vocab = None
    for i in range(0, len(image_paths)):
        # 读取图像
        im = imread(image_paths[i])
        ...
        out = np.concatenate((out, get_hogs(im, cells_per_block, pixels_per_cell, orients)))

    # Kmeans聚类，得到聚类簇的中心为词袋模型中的词
    kmeans = KMeans(vocab_size, max_iter=100, tol=1e-4, random_state=0).fit(out)
...
def get_bags_of_words(image_paths):
    ...
    hist = np.zeros((len(image_paths), len(vocab)))
    for i in range(0, len(image_paths)):
        # 读取图像
        im = imread(image_paths[i])
        # 转换为灰度图
        ...
        out = get_hogs(im, cells_per_block, pixels_per_cell, orients)

        # 创建直方图
        for j in range(0, len(out)):
            dist = cdist(np.array([out[j]]), vocab, metric='cosine')
            bin = np.argsort(dist)[0][0]
            hist[i, bin] = hist[i, bin] + 1

        # 标准化直方图
        norm = np.linalg.norm(hist[i])
        if norm != 0:
            hist[i] = hist[i] / norm
...
def svm_classify(train_image_feats, train_labels, test_image_feats):
    ...
    # 训练SVM分类器
    classifier = LinearSVC('l2', multi_class='ovr', random_state=0, max_iter=100)
    # 用训练得到的SVM分类器预测结果
    classifier.fit(train_image_feats, train_labels)

    print('*******')
    print(train_image_feats.shape)
    print(test_image_feats[0].shape)

    out = [''] * len(test_image_feats)
    for i in range(0, len(test_image_feats)):
        out[i] = classifier.predict(test_image_feats[i].reshape(1, -1))
...
def nearest_neighbor_classify(train_image_feats, train_labels, test_image_feats):
    ...
    labels = [None] * len(test_image_feats)
    for i in range(0, len(test_image_feats)):
        nn = np.argsort(distances[i])
        nn_label = np.array(train_labels)
        nn_label = nn_label[nn[:k]]
